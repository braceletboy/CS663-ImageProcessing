function enlarged_image = myBilinearInterpolation()
%% Perform Bilinear Interpolation on the given image.
%
% SYNTAX:
%   enlarged_image = myBilinearInterpolation()
%
% OUTPUT:
...
for i = 1:3*num_rows-2
    for j = 1:2*num_columns-1
        ...
        elseif rem(i,3)==2
            if rem(j,2)==1
                enlarged_image(i,j) = (2/3*enlarged_image(i-1,j) + ...
                    1/3*enlarged_image(i+2,j));
            else
                enlarged_image(i,j) = (1/3*enlarged_image(i-1,j-1) + ...
                    1/3*enlarged_image(i-1,j+1) + ...
                    1/6*enlarged_image(i+2,j-1) + ...
                    1/6*enlarged_image(i+2,j+1));
            end
        else
            if rem(j,2)==1
                enlarged_image(i,j) = (1/3*enlarged_image(i-2,j) + ...
                    2/3*enlarged_image(i+1,j));
            else
                enlarged_image(i,j) = (1/3*enlarged_image(i+1,j-1) + ...
                    1/3*enlarged_image(i+1,j+1) + ...
                    1/6*enlarged_image(i-2,j-1) + ...
